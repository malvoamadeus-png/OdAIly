...
def test_console_tasks_order_mixed_sqlite_timestamps_chronologically(monkeypatch, tmp_path):
    path = configure(monkeypatch, tmp_path)
    with connect_sqlite(path) as conn:
        conn.executemany(
            """
            INSERT INTO tasks(source, source_item_id, title, content, status, created_at)
            VALUES (?, ?, ?, ?, ?, ?)
            """,
            [
                ("x", "x-old", "older ISO task", "body", "discarded", "2026-08-05T08:39:31.036125+00:00"),
                ("panews", "news-new", "newer SQLite task", "body", "auto_published", "2026-08-05 09:11:21"),
                ("jinse", "news-same-second", "same-second task", "body", "ready_review", "2026-08-05 09:11:21"),
            ],
        )
        conn.commit()
    ...
    assert [row["title"] for row in rows] == ["same-second task", "newer SQLite task", "older ISO task"]
