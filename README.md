...
Hyperliquid notifications now use a dual-threshold policy by default:
single fill `>= 500000` USD alerts immediately; otherwise fills enter a rolling
`600` second window and a single aggregate alert is sent when the window first
crosses `1000000` USD.
...
DASHSCOPE_API_KEY=
BLOCKBEATS_API_KEY=
BLOCKBEATS_AUTO_REGISTER_ENABLED=true
BLOCKBEATS_REGISTRATION_TIMEOUT_SECONDS=120
...
X_PROCESS_PUBLISHER_REASONING_EFFORT=medium
...
Compatibility units for the retired `odaily-x-process@` and
`odaily-external-media-alert@` staged workers remain in `deploy/` for disaster
rollback only. Do not enable them in the current local-pipeline production
topology. Writer3 units remain disabled unless `WRITER3_ENABLED=true` is an
