...
- 最终写作：使用 `MEME_FAST_WRITER_MODEL`，默认固定为 `gpt-5.6-luna`。Meme 叙事不再调用 Grok、Grok X Search、Grok 实体补充或 GMGN 叙事。`MEME_PRICE_SOURCE=gmgn` 的市场价格适配器是独立链路，仍可继续使用。
- 正文整理复用 OdAIly 的 `ODAILY_LLM_BASE_URL`、`ODAILY_LLM_API_KEY`，模型仅由 `MEME_FAST_WRITER_MODEL` 覆盖。
