...
网页和插件只访问 `editor-plugin-api-server`：本地单账号认证签发不透明 Bearer session，普通控制台业务表由 `/console/data` 白名单接口访问主 SQLite，快讯运营跨表查询使用 `/console/newsflash-operations`，插件信息流由本地 feed store 提供。浏览器不持有数据库地址或数据库密钥。
