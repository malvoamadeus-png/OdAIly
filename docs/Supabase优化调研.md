...
- `editor_plugin_feed` 可以在不改变插件展示范围的前提下优化内部取数方式：各来源先分别筛选最近 30 分钟并限制候选数量，再合并排序。
...
- 最近窗口：业务只关心最近一段时间的数据，例如插件信息流只展示最近 30 分钟。
...
- `editor_plugin_feed` 从多张业务表里取最近 30 分钟内可以展示成卡片的内容。
...
- 如果某些来源表已经很大，而查询条件没有很好利用索引，就可能为了找最近 30 分钟的数据读很多历史数据。
- 如果有多个编辑同时开着插件，请求会叠加。

可讨论优化：

- 保持插件轮询频率不变，但优化数据库函数内部取数方式。
- 先让每个来源各自只取“最近 30 分钟 + 有展示价值 + 数量上限”的少量候选，再把这些候选合并排序。
- 对应补充更贴合这些筛选条件的索引。
...
```text
每个来源先自己从最近 30 分钟里挑一小篮子，
例如新快讯最多取 80 条、审核者最多取 40 条、此前消息最多取 40 条、巨鲸最多取 40 条，
然后再把这些小篮子合并排序。
```

这样做的目标不是改变插件看到的业务范围；插件仍然看最近 30 分钟的信息流。目标是让数据库少翻历史数据、少排序无关数据。
...
- worker 会频繁连接数据库、claim 任务、更新状态、写 heartbeat。
- 如果出现长事务、连接堆积或锁等待，会推高 CPU/memory。
- 部分处理 worker 会额外保留数据库 `LISTEN` 连接，用于收到任务或 Prompt 配置变化时立即醒来。
- systemd 如果配置或部署异常，可能出现重复实例；重复实例会放大数据库连接数和写入频率。

可讨论优化：

- 只读查看当前连接数、等待事件和是否存在长事务。
- 确认 worker 是否有异常重启或重复实例。
- 必要时再讨论连接池、超时、批量写入。

当前理解：

- Linux worker 是当前系统必备运行面，不以停掉 worker 作为优化方向。
- worker 优化优先看“是否重复、是否卡住、是否长事务、是否连接过多”，不是删功能。
- X 抓取和外媒/AI信源收集的配置生效主要按低频重载配置理解；这种配置轮询本身通常不是连接数压力的大头。
- `heartbeat` 是 worker 定期把“我还活着/是否成功/错误信息”写入数据库，不等于一直占用一个监听窗口。
- `LISTEN` 是另一种机制：处理类 worker 可以保留数据库监听连接，等待任务队列或 Prompt 配置变化通知。
...
- `pipeline_worker_heartbeats`：确认 worker 心跳是否正常、是否有重复 worker_id。
...
- 处理类 worker：从数据库领取待处理任务，调用判断、搜索、编写、发布、审核、提醒等组件，把任务一步步推进。
...
- 数据库 LISTEN：worker 保留一个监听连接，等数据库通知它“有新任务/Prompt 变了”，主要出现在处理类 worker。

可讨论替代方案：

- 保持当前模式：控制台写数据库，worker 定时重载配置。优点是简单、安全，前端不需要直接接触服务器执行权限。
- 配置变更后通过数据库通知或版本号让 worker 更快重载。优点是仍以数据库为边界，不新增公开 Linux 控制接口。
...
- `editor_plugin_feed` 已改为每个来源先按最近 30 分钟和候选上限取数，再进入原有合并排序。
