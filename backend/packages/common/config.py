...
def _llm_auditor_model(*specific_names: str) -> str:
    for name in specific_names:
        value = os.getenv(name)
        if value:
            return value
    return DEFAULT_GPT_AUDITOR_MODEL
...
def load_x_processing_settings() -> XProcessingSettings:
    ...
    payload = {
        ...
        "search_cache_short_retention_days": int(os.getenv("SEARCH_CACHE_SHORT_RETENTION_DAYS") or 2),
        "search_cache_reference_retention_days": int(
            os.getenv("SEARCH_CACHE_REFERENCE_RETENTION_DAYS") or 8
        ),
...
class AuditorSettings(BaseModel):
    ...
    model: str = DEFAULT_GPT_AUDITOR_MODEL
...
def load_auditor_settings() -> AuditorSettings:
    ...
    auditor_model = _llm_auditor_model("AUDITOR_MODEL")
    payload = {
        "openai_api_key": (
