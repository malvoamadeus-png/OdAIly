...
1. 结果前置型
    ...
    结果前置只调整信息顺序，不得牺牲主体关键身份或事件来源。若主体因治理提案、财库提取、解锁接收、
    项目方/基金会关联、黑客身份、私募地址等事实获得新闻价值，标题必须保留该身份，不得弱化为“某地址”。
...
3. 主体特征放大型
    ...
    - 单场购入百万美元结果份额，新生巨豪建仓40万美元XX战胜XX

    禁止：虚构主体特征、推断主体动机、使用情绪化形容词

【可选的表达优化】
如果标题前后两个分句出现相同核心名词，可在不改变事实含义的前提下考虑改用更自然的动词或表达，避免重复。例如前句已使用“空头持仓”，后句可考虑使用“做空 BTC、ETH”。该规则不是强制要求，只有在改写自然且术语含义保持准确时才采用。"""
