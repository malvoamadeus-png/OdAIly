...
DEFAULT_REGULAR_ALLOW_RULES = [
    PublisherRule(
        ...
        name="链上大额 / 巨鲸 / 清算",
        description=(
            "放行链上大额转账、巨鲸或具名关系主体的地址动作、持仓或增减持、储备变化、清算、解锁后出售等"
            "有明确事实和主体的链上资金事件。不要求正文必须给出完整 0x 地址；团队关联实体、项目方钱包、"
            "基金会相关地址、做市商相关钱包等可识别关系主体也可作为主体。"
        ),
        enabled=True,
        examples=[
    ...
    PublisherRule(
        ...
        name="安全 / 事故 / 停运",
        description=(
            "放行黑客攻击、被盗、漏洞、链或服务事故、重大宕机、停运等风险事件。金额型安全损失通常需达到"
            " 10 万美元及以上；低于 10 万美元时，只有链停摆、核心协议级漏洞、持续攻击或广泛用户资产风险等"
            "重大非金额影响才可放行。"
        ),
        enabled=True,
        examples=[
...
DEFAULT_REGULAR_DENY_RULES = [
    ...
    PublisherRule(
        ...
        name="小额安全损失",
        description=(
            "排除金额型安全事件中损失低于 10 万美元、且没有链停摆、核心协议级漏洞、持续攻击或广泛用户资产风险等"
            "重大非金额影响的内容，默认挂后台人工处理。"
        ),
        enabled=True,
        examples=[
            "某协议因整数转换漏洞损失 2.49 万枚 DAI",
            "某小型项目被盗约 6 万美元且未见持续风险",
        ],
    ),
    PublisherRule(
