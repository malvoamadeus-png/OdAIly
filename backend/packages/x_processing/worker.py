...
只允许两种 route：
- non_mainstream_media：保留并进入统一 Crypto信源写作模板。
...
class XProcessingWorker:
    ...
    def _build_ai_client(self) -> TextGenerationClient:
        ...
        return OpenAIResponsesClient(
            ...
            timeout_seconds=self.settings.writer_request_timeout_seconds,
...
def build_writer_prompt(
    *,
    ...
    if is_non_mainstream_media_task(task) or is_ai_source_task(task):
        ...
        is_ai_source = is_ai_source_task(task)
        site_display_name = task.metadata.get("site_display_name") or ("AI信源" if is_ai_source else "Crypto信源")
        block_label = "待处理AI信源原文" if is_ai_source else "待处理Crypto信源原文"
        return (
            f"{render_prompt_content(prompt)}\n\n"
            f"【{block_label}】\n"
