...
import {
  ...
  type ContributionsMonthlyPayload,
  type EventPage,
  type FirstPublication,
  type NewsflashPage,
  type NewsflashRow,
  type Person,
  type QualityItem,
  ...
  return (
    <section className="newsflashSection">
      <form className="newsflashFilters" onSubmit={(event) => { event.preventDefault(); void load(1); }}>
        ...
        <label><span>首发状态</span><select value={filters.first_status} onChange={(event) => setFilters({ ...filters, first_status: event.target.value })}><option value="">全部</option><option value="ready">已计算</option><option value="unmatched">未匹配事件</option><option value="insufficient">数据不足</option><option value="excluded">事件已排除</option></select></label>
...
function Quality({ refreshToken, onError }: { refreshToken: number; onError: (value: string) => void }) {
  ...
  return <section className="newsflashSection qualitySection">
    ...
    {view === 'week' && data && <details className="qualityRules"><summary>查看判定规则与当周冻结信源范围</summary><div className="qualityRuleGrid"><section><h3>排除词组（{data.rules.keyword_groups.length}）</h3><div className="ruleTagList">{data.rules.keyword_groups.map((group) => <span key={group.key}>{group.label}</span>)}</div><p>组内词汇需同时出现，按词组标注的标题、正文或标题和正文范围匹配；任一词组命中即排除。</p></section><section><h3>固定常规信源（{data.rules.regular_source_accounts.length}）</h3><div className="ruleTagList">{data.rules.regular_source_accounts.map((account) => <a key={account} href={`https://x.com/${account}`} target="_blank" rel="noreferrer">@{account}</a>)}</div></section><section><h3>当周 X 自动覆盖（{data.rules.automated_x_accounts.length}）</h3><div className="ruleTagList">{data.rules.automated_x_accounts.length ? data.rules.automated_x_accounts.map((account) => <span key={account}>@{account}</span>) : <span>无</span>}</div></section><section><h3>当周媒体自动覆盖（{data.rules.automated_media_domains.length}）</h3><div className="ruleTagList">{data.rules.automated_media_domains.map((domain) => <span key={domain}>{domain}</span>)}</div></section></div><p>规则快照：{localDate(data.rules.snapshot_at)}。浏览量、贡献状态、首发、排班和人工覆盖仍按当前数据实时重算。</p></details>}
  </section>;
}
...
function Contributions({ people, refreshToken, onError, onMessage }: { people: Person[]; refreshToken: number; onError: (value: string) => void; onMessage: (value: string) => void }) {
  ...
  const [monthlyData, setMonthlyData] = useState<ContributionsMonthlyPayload | null>(null);
  const [loading, setLoading] = useState(false);
  ...
  async function loadMonth() {
    setLoading(true);
    onError('');
    try { setMonthlyData(await newsflashOperations<ContributionsMonthlyPayload>('contributions_month', { report_month: month })); }
    catch (reason) { onError(reason instanceof Error ? reason.message : String(reason)); }
    finally { setLoading(false); }
  }
  useEffect(() => { void (mode === 'week' ? loadWeek() : loadMonth()); }, [mode, week, month, refreshToken]);
  async function patch(sourceItemId: string, patchValue: Record<string, unknown>) { try { await newsflashOperations('update', { source_item_id: sourceItemId, patch: patchValue }); onMessage(`快讯 ${sourceItemId} 已保存`); await loadWeek(); } catch (reason) { onError(reason instanceof Error ? reason.message : String(reason)); } }
  ...
  return <section className="newsflashSection">
    <div className="sectionCommandBar"><div className="segmentedControl" aria-label="贡献快讯显示范围"><button type="button" className={mode === 'week' ? 'active' : ''} onClick={() => setMode('week')}>按周明细</button><button type="button" className={mode === 'month' ? 'active' : ''} onClick={() => setMode('month')}>月度总览</button></div>{mode === 'week' ? <div className="weekPicker"><label><span>统计周</span><input type="date" value={week} onChange={(event) => setWeek(mondayKey(new Date(`${event.target.value}T12:00:00+08:00`)))} /></label><span className="periodHint">{formatWeekLabel(week)}</span></div> : <label><span>统计月份</span><input type="month" value={month} onChange={(event) => setMonth(event.target.value)} /></label>}{mode === 'week' && data?.in_progress && <span className="statusText ready">本周进行中</span>}{mode === 'month' && monthlyData?.in_progress && <span className="statusText ready">当月包含进行中的周</span>}</div>
    {loading && <div className="emptyInline">正在加载贡献快讯数据…</div>}
    {!loading && mode === 'month' && monthlyData && <ContributionMonthlyOverview data={monthlyData} />}
    {!loading && mode === 'week' && <>
...
function monthlyAverage(value: number | null, viewCoverage: { known: number; total: number }) {
  if (value == null) return '—';
  const known = coverage(viewCoverage);
  return <>{value.toLocaleString()} {known && <small>{known}</small>}</>;
}
...
const firstSourceLabels: Record<string, string> = {
  odaily: 'Odaily',
...
function FirstSourceSelect({ value, onChange }: { value: FirstPublication; onChange: (value: string | null) => void }) {
  return <select className="inlineSelect firstSourceSelect" aria-label="首发单位" value={value.override_source || ""} onChange={(event) => onChange(event.target.value || null)}><option value="">自动（{value.label}）</option>{Object.entries(firstSourceLabels).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select>;
}
